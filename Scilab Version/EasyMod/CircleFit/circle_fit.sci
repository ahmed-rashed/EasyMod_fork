function [freq_local,H_local,H_gen_local,infoMODE,circ_prop] = circle_fit(H,frq,fmin,fmax) ;
...
//
//  Synthax:
//  [freq_local,H_local,H_gen_local,infoMODE,circ_prop] = circle_fit(H,frq,fmin,fmax)
//
//  Input data:
...
freq_local = frq(index_low:index_high) ;
...
if N_pts < 5
  ...
  theta_brut = atan(y-y0,x-x0) ;
  theta_brut = pmodulo(theta_brut,2*%pi) ;
  ...
  if isempty(ind_zero)==%T
      disp('!!!')
      disp('No resonance in the studied frequency range')
      disp(' ')
      problem = 1 ;
  else
     [fr, theta_r] = estimate_natural_freq(ind_zero,delta2,f_delta2,theta_delta2) ;
