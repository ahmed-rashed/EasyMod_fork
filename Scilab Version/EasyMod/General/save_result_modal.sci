...
//            infoFRF(jj).infoMODE(index_mode,1) = natural frequency 
//            identified on the FRF(jj) for the mode 'index_mode'
//            infoFRF(jj).infoMODE(index_mode,2) = damping identified on the
//            FRF(jj) for the mode 'index_mode 
//            infoFRF(jj).infoMODE(index_mode,3) = modal constant
//            identified on the FRF(jj) for the mode 'index_mode.
//
//  Output data:
...
[M,N] = size(infoFRF) ;
Nbr_frf = M ;
[M,N] = size(infoFRF(1).infoMODE(:,1)) ;
Nbr_mode = M ;
Nbr_FRF_estime = ones(Nbr_mode,Nbr_mode)*Nbr_frf ;
for i = 1:Nbr_frf
    for j = 1:Nbr_mode
        if isnan(infoFRF(i).infoMODE(j,1))
            infoFRF(i).infoMODE(j,1) = 0 ;
            infoFRF(i).infoMODE(j,2) = 0 ;
