...
for ii = 1:Nexp1
    for jj = 1:Nexp2
        DELTA_FREQ(ii,jj) = (abs(infoMODE1.frequencyk(ii)-infoMODE2.frequencyk(jj)))/(infoMODE1.frequencyk(ii)) ;
         MAC(ii,jj) = real(abs(sum(psi1(:,ii).*conj(psi2(:,jj))))^2/((sum(psi1(:,ii).*conj(psi1(:,ii))))*(sum(psi2(:,jj).*conj(psi2(:,jj)))))) ;
        if (abs(MAC(ii,jj)) > macinf) & (abs(DELTA_FREQ(ii,jj)) <= delfreqsup)
            compteur = compteur+1 ;
            coupled(compteur,1) = ii ;
            coupled(compteur,2) = jj ;
        elseif (abs(MAC(ii,jj)) > macinf) & (abs(DELTA_FREQ(ii,jj)) > delfreqsup)
               compteur = compteur+1 ;
               coupled(compteur,1) = ii ;
               coupled(compteur,2) = jj ;
        end                 
...
if graph == 1
    [l c] = size(MAC) ;
    figure ;
    hist3d(list(real(MAC),(0.5:l+0.5),(0.5:c+.5)));
    title('MAC matrix') ;
    xlabel('Mode shapes of analysis 1') ;
    ylabel('Mode shapes of analysis 2') ;
    zlabel('MAC value') ;
end
